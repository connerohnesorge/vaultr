...
Configuration is environment only: `SEAL_BROKER_LISTEN`, `SEAL_BROKER_BUCKET`,
`SEAL_BROKER_SPOOL`, `SEAL_BROKER_SEAL_FILES`, `SEAL_BROKER_MAX_OBJECT_BYTES`,
`SEAL_BROKER_TAILSCALE_SOCKET`, `SEAL_BROKER_DEV_TENANT`.
...
- **Bodies spool to disk before they are stored.** That buys an exact length to
  ...
  2.88 GB, so the spool volume is a deployment input, not an afterthought.
