                ...
                let svc = hyper::service::service_fn(|req: Request<Incoming>| async move {
                    ...
                    Ok::<_, Infallible>(
                        Response::builder()
                            ...
                            .body(Full::new(Bytes::from(zstd::encode_all(SSE.as_bytes(), 1).unwrap())))
                            .unwrap(),
                    )
                });
...
fn start_proxy(mut adapter: crate::adapter::Adapter, upstream: String, vault: &std::path::Path, otel: Arc<crate::otel::Otel>) -> u16 {
    ...
    tokio::spawn(async move {
        ...
        tx.send(port).unwrap();
        let ctx = Arc::new(ProxyCtx { adapter, vault, client: crate::http_client(), otel });
...
pub async fn self_test() {
    ...
    assert!(crate::herdr::pane_list().await.is_none(), "absent herdr socket must fail soft");
    ...
    let otlp_listener = tokio::net::TcpListener::bind(("127.0.0.1", 0)).await.unwrap();
    let otlp_port = otlp_listener.local_addr().unwrap().port();
    {
        let exports = exports.clone();
        tokio::spawn(async move {
            loop {
                let Ok((stream, _)) = otlp_listener.accept().await else { break };
                let exports = exports.clone();
                tokio::spawn(async move {
                    let io = hyper_util::rt::TokioIo::new(stream);
                    let svc = hyper::service::service_fn(move |req: Request<Incoming>| {
                        ...
                        async move {
                            ...
                            exports.lock().unwrap().push(json!({ "path": path, "authorization": auth, "body": body }));
    ...
    std::env::set_var("VAULTR_OTEL_ENDPOINT", format!("http://127.0.0.1:{otlp_port}"));
    ...
    msg(json!([{ "role": "user", "content": "one" }]), json!([{ "name": "t1" }])).await;
    msg(json!([{ "role": "user", "content": "one" }, { "role": "assistant", "content": "r" }]), json!([{ "name": "t1" }])).await;
    // compaction: rewritten shorter history must be captured losslessly
    msg(json!([{ "role": "user", "content": "SUMMARY" }]), json!([{ "name": "t1" }])).await;
    // capture runs async post-stream; give it a beat
    tokio::time::sleep(std::time::Duration::from_millis(300)).await;

    let dir = session_dir(&vault, sid).unwrap();
    assert!(!dir.join("herdr.jsonl").exists(), "absent herdr socket must not create a sidecar");
    ...
    assert_eq!(turns[0]["request"]["body_delta"]["history"]["prefix_length"], 0);
    assert!(turns[0]["request"]["body_delta"]["set"].get("tools").is_some(), "big field stored on first turn");
    assert_eq!(turns[1]["request"]["body_delta"]["history"]["prefix_length"], 1);
    assert!(turns[1]["request"]["body_delta"]["set"].get("tools").is_none(), "unchanged big field omitted");
    assert!(turns[1]["request"]["body_delta"]["set"].get("model").is_some(), "small field verbatim every turn");
    assert_eq!(turns[2]["request"]["body_delta"]["history"]["prefix_length"], 0, "compaction detected via LCP");
    assert_eq!(turns[2]["request"]["body_delta"]["history"]["append"].as_array().unwrap().len(), 1);
    assert_eq!(turns[0]["response"]["complete"], true);
    assert_eq!(turns[0]["response"]["headers"]["request-id"], "req_test");
    assert!(turns[0]["response"]["headers"].get("x-secret").is_none(), "non-allowlisted header dropped");

    let state: Value = serde_json::from_str(&std::fs::read_to_string(dir.join("state.json")).unwrap()).unwrap();
    assert_eq!(state["schema_version"], 1);
    assert_eq!(state["request_body"]["messages"], json!([{ "role": "user", "content": "SUMMARY" }]));

    let meta: Value = serde_json::from_str(&std::fs::read_to_string(vault.join(".meta").join(format!("{sid}.json"))).unwrap()).unwrap();
    assert_eq!(meta["model"], "m");
    assert_eq!(meta["harness"], "claude-code");

    // -- codex adapter (zstd + header identity) --
    let codex_port = start_proxy({ let mut v = ads; v.remove(1) }, upstream.clone(), &vault, otel.clone());
    ...
    assert!(cturns[0]["request"]["body_delta"]["set"].get("instructions").is_some());
    assert_eq!(cturns[0]["request"]["body_delta"]["history"]["key"], "input");
    ...
    assert!(exports.iter().all(|e| e["authorization"] == "Bearer test-token"));
    let point_attrs = |point: &Value| -> Value {
        let mut m = serde_json::Map::new();
        for a in point["attributes"].as_array().unwrap() {
            let v = &a["value"];
            let val = v.get("stringValue").cloned()
    ...
    let metrics = metric_export["body"]["resourceMetrics"][0]["scopeMetrics"][0]["metrics"].as_array().unwrap();
    let token_points = metrics.iter().find(|m| m["name"] == "vaultr.tokens").unwrap()["sum"]["dataPoints"].as_array().unwrap();
    let claude_input = token_points.iter().find(|p| {
        let a = point_attrs(p);
        a["type"] == "input" && a["harness"] == "claude-code" && a["model"] == "m"
    }).unwrap();
    let codex_cache = token_points.iter().find(|p| {
        let a = point_attrs(p);
    ...
    assert_eq!(log_export["body"]["resourceLogs"][0]["scopeLogs"][0]["logRecords"].as_array().unwrap().len(), 4);
    assert_eq!(point_attrs(&log_export["body"]["resourceLogs"][0]["resource"])["loki.resource.labels"], "service.namespace");

    // -- 502 path --
    let broken_port = start_proxy(adapters().remove(0), "http://127.0.0.1:1".into(), &vault, otel.clone());
    let status = client
    ...
    // -- scrub: regex pattern redacted, session UUID left intact (the FP gitleaks caused) --
    {
        ...
        std::fs::write(&sf, json!({ "out": format!("token={tok} sid={uuid}") }).to_string() + "\n").unwrap();
        ...
        assert!(scrubbed.contains(uuid), "session UUID must survive scrub (was a gitleaks false positive)");
        ...
        std::fs::write(home.join(".config/wireproxy/scrub-denylist.txt"), format!("{plain}\n")).unwrap();
        let df = vault.join("scrub-denylist-test.jsonl");
        std::fs::write(&df, json!({ "out": format!("pw={plain}") }).to_string() + "\n").unwrap();
    ...
    // -- herdr: socket round-trip, exact binding, sibling shape, and dedupe --
    {
        ...
        assert_eq!(snapshots[0]["siblings"][0]["agent_session_id"], "sibling-session");
