...
/// to [`reconstruct`], and memory is bounded by the largest physical record.
pub fn for_each_envelope(path: &Path, mut visit: impl FnMut(&Value)) -> Result<()> {
...
impl ReconState {
    ...
    fn apply(&mut self, env: &Value) {
        ...
        self.harness = Harness::from_envelope(env, self.harness);
