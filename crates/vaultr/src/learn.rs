...
pub fn parse_record(text: &str) -> Result<Pass> {
    let value: serde_json::Value =
        serde_json::from_str(text).context("record is not JSON")?;
...
mod tests {
    ...
    fn latest_pass_wins_for_one_learner() {
        ...
        record(dir.path(), "learn-claude-mac-20260101T000000Z.json", "2026-01-01T00:00:00Z");
        record(dir.path(), "learn-claude-mac-20260202T000000Z.json", "2026-02-02T00:00:00Z");
    ...
    fn unknown_learner_filename_is_rejected() {
        let dir = tempfile::tempdir().unwrap();
        record(dir.path(), "learn-gemini-mac-20260101T000000Z.json", "2026-01-01T00:00:00Z");
    ...
    fn malformed_record_is_an_error_naming_its_path() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("learn-claude-mac-20260101T000000Z.json"), "{}").unwrap();
        let error = format!("{:#}", session_passes(dir.path()).unwrap_err());
        assert!(error.contains("learn-claude-mac-20260101T000000Z.json"), "{error}");
    }
